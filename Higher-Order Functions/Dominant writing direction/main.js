...
function characterScript(code) {
  ...
  function dominantDirection(text) {
    let scriptsDirections = countBy(text, char => {
        let script = characterScript(char.codePointAt(0));
        return script ? script.direction : "none";
      }).filter(({direction}) => direction != "none");
      let scriptName = "", scriptCount = 0;
      for (let i = 0; i < scriptsDirections.length; i++) {
        if (scriptsDirections[i].count > scriptCount) {
            scriptName = scriptsDirections[i].name;
            scriptCount = scriptsDirections[i].count;
        }
      }
    return scriptName;
  }
